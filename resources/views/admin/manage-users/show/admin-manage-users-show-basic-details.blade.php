    ...
    <x-layout.details.more-details class="mt-8">
        <x-layout.details.more-details.section title="Personal Details" title_text_color="primary">
            <div class="space-y-2">
                ...
                <x-layout.details.more-details.data-field field="Full Name" value="{{ $user->profile->first_name . ' ' . $user->profile->middle_name . ' ' . $user->profile->surname . ' ' . $user->profile->suffix }}" />
            ...
            <div class="space-y-2">
                <div class="flex gap-2 break-words w-full">
                    ...
                    <p class="text-base font-bold w-2/3 cursor-pointer hover:underline" 
                        @click="navigator.clipboard.writeText('{{ $user->kyc?->liveness_req_id ?? '' }}'), alert('Liveliness Request ID copied to clipboard!')">
                        {{ $user->kyc?->liveness_score ?? 'N/A' }}
                    </p>
                </div>
                <div class="flex gap-2 break-words w-full">
                    <p class="text-base w-1/3">Selfie Sanity Score</p>
                    <p class="text-base font-bold w-2/3 cursor-pointer hover:underline" 
                        @click="navigator.clipboard.writeText('{{ $user->kyc->selfie_sanity_req_id ?? '' }}'), alert('Selfie Sanity Request ID copied to clipboard!')">
                        {{ $user->kyc?->selfie_sanity_score ?? 'N/A' }}
                    </p>
                </div>
                <div class="flex gap-2 break-words w-full">
                    <p class="text-base w-1/3">Card Sanity Score</p>
                    <p class="text-base font-bold w-2/3 cursor-pointer hover:underline" 
                        @click="navigator.clipboard.writeText('{{ $user->kyc?->card_sanity_req_id ?? '' }}'), alert('Card Sanity Request ID copied to clipboard!')">
                        {{ $user->kyc?->card_sanity_score ?? 'N/A' }}
                    </p>
                </div>
                <div class="flex gap-2 break-words w-full">
                    <p class="text-base w-1/3">Card Tampering Score</p>
                    <p class="text-base font-bold w-2/3 cursor-pointer hover:underline" 
                        @click="navigator.clipboard.writeText('{{ $user->kyc->card_tampering_req_id ?? '' }}'), alert('Card Tampering Request ID copied to clipboard!')">
                        {{ $user->kyc?->card_tampering_score ?? 'N/A' }}
                    </p>
                </div>
            </div>
        </x-layout.details.more-details.section>
        <x-layout.details.more-details.section title="Pictures" title_text_color="primary">
            ...
            @else
                <p class="text-red-400" >This user did not finish the KYC process.</p>
            @endif
        </x-layout.details.more-details.section>
