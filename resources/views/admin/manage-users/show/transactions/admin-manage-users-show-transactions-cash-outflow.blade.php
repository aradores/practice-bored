                        ...
                        <x-slot:table_data>
                            ...
                            @foreach ($cash_outflows as $key => $cash_outflow)
                                ...
                                wire:click="handleTableRowClick('{{ $cash_outflow->txn_no }}')" wire:key='cash_outflow-{{ $key }}'>
                                    ...
                                    <x-table.rounded.td>
                                        {{ $cash_outflow->recipientName }}
                                    </x-table.rounded.td>
                                    <x-table.rounded.td class="text-primary-600 font-bold">{{ Number::currency($cash_outflow->total_amount, 'PHP') }}</x-table.rounded.td>
                                    <x-table.rounded.td>{{ $cash_outflow->created_at->timezone('Asia/Manila')->format('M d, Y') }}</x-table.rounded.td>
                                    <x-table.rounded.td>{{ $cash_outflow->txn_no }}</x-table.rounded.td>
                                    <x-table.rounded.td>
