<x-main.content class="!px-16 !py-10" x-data="{
    ...
    {{-- Filters --}}
    <div class="grid xl:grid-cols-3 2xl:grid-cols-5 gap-3 mb-8">
        <button wire:click="handleFilterBoxClick('')"
            class="{{ $activeBox === '' ? 'bg-purple-gradient-to-right shadow-lg z-10 text-white' : 'bg-white hover:shadow-lg transition-all text-primary-600' }} flex flex-row items-center justify-between cursor-pointer px-4 py-6 rounded-1 break-words">
            <div class="flex items-center gap-2 2xl:gap-4">
                <x-icon.user width="24" height="24" fill="{{ $activeBox === '' ? '#ffff' : '#7f56d9' }}" />
                <p class="{{ $activeBox === '' ? 'text-white' : 'text-rp-neutral-600' }}">All users</p>
            </div>
            <span class="font-bold">{{ $this->allUsersCount }}</span>
        </button>

        <button wire:click="handleFilterBoxClick('pending')"
            class="{{ $activeBox === 'pending' ? 'bg-purple-gradient-to-right shadow-lg z-10 text-white' : 'bg-white hover:shadow-lg transition-all text-primary-600' }} flex flex-row items-center justify-between cursor-pointer px-4 py-6 rounded-1 break-words">
            <div class="flex items-center gap-2 2xl:gap-4">
                <x-icon.hourglass width="24" height="24" fill="{{ $activeBox === 'pending' ? '#ffff' : '#7f56d9' }}" />
                <p class="{{ $activeBox === 'pending' ? 'text-white' : 'text-rp-neutral-600' }}">Pending</p>
            </div>
            <span class="font-bold">{{ $this->pendingUsersCount }}</span>
        </button>

        <button wire:click="handleFilterBoxClick('verified')"
            class="{{ $activeBox === 'verified' ? 'bg-purple-gradient-to-right shadow-lg z-10 text-white' : 'bg-white hover:shadow-lg transition-all text-primary-600' }} flex flex-row items-center justify-between cursor-pointer px-4 py-6 rounded-1 break-words">
            <div class="flex items-center gap-2 2xl:gap-4">
                <x-icon.check width="24" height="24" fill="{{ $activeBox === 'verified' ? '#ffff' : '#7f56d9' }}" />
                <p class="{{ $activeBox === 'verified' ? 'text-white' : 'text-rp-neutral-600' }}">Active</p>
            </div>
            <span class="font-bold">{{ $this->verifiedUsersCount }}</span>
        </button>

        <button wire:click="handleFilterBoxClick('denied/deactivated')"
            class="{{ $activeBox === 'denied/deactivated' ? 'bg-purple-gradient-to-right shadow-lg z-10 text-white' : 'bg-white hover:shadow-lg transition-all text-primary-600' }} flex flex-row items-center justify-between cursor-pointer px-4 py-6 rounded-1 break-words">
            <div class="flex items-center gap-2 2xl:gap-4">
                <x-icon.close-filled fill="{{ $activeBox === 'denied/deactivated' ? '#ffff' : '#7f56d9' }}" />
                <div
                    class="{{ $activeBox === 'denied/deactivated' ? 'text-white' : 'text-rp-neutral-600' }} flex flex-col 2xl:flex-row 2xl:gap-2 text-left">
                    <span>Denied /</span>
        ...
        <a href="{{ route('admin.manage-users.requests.index') }}"
            ...
            <div class="flex items-center gap-2 2xl:gap-4">
                <div
                    class="{{ isset($is_active_page) ? 'text-white' : 'text-rp-neutral-600' }} flex flex-col 2xl:flex-row 2xl:gap-2 text-left">
            ...
            <x-slot:table_header>
                <x-table.standard.th class="w-max">
                    <x-input type="checkbox" wire:model.live="selectAll"
                        wire:change="handleSelectAllCheckbox($event.target.checked, {{ $users->getCollection() }})" />
                </x-table.standard.th>
                @if (count($checkedUsers) > 0)
                    ...
                    <x-table.standard.th class="min-w-64 max-w-64 2xl:min-w-80 2xl:max-w-80">
                        <div class="flex items-center gap-2">
                            <x-button.filled-button size="sm"
                                @click="groupConfirmationModal.actionType='activate';groupConfirmationModal.visible=true"
                                color="primary" class="w-1/2" size="sm">activate</x-button.filled-button>
                            <x-button.outline-button size="sm"
                                @click="groupConfirmationModal.actionType='deactivate';groupConfirmationModal.visible=true"
                                color="primary" class="w-1/2" size="sm">deactivate</x-button.outline-button>
                        </div>
                    </x-table.standard.th>
                @else
                    <x-table.standard.th class="w-52 min-w-52 max-w-52">
                ...
                @foreach ($users as $key => $user)
                    <x-table.standard.row wire:key="{{ $user->id }}">
                        <x-table.standard.td class="w-max">
                            ...
                            <x-input type="checkbox" wire:model.live="checkedUsers" value="{{ $user->id }}"
                                wire:change="handleSingleSelectCheckbox({{ $users->getCollection() }})" />
                        </x-table.standard.td>
                        <x-table.standard.td class="w-52 min-w-52 max-w-52">
                            <a href="{{ route('admin.manage-users.show.basic-details', $user->id) }}"
                        ...
                        @switch($user->profile->status)
                            @case('pending_update')
                                <x-table.standard.td class="w-32 min-w-32 max-w-32">
                                    <x-status color="yellow">Pending Update</x-status>
                                </x-table.standard.td>
                                ...
                                <x-table.standard.td class="min-w-52 max-w-52 2xl:min-w-80 2xl:max-w-80">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <div class="flex items-center w-full gap-1">
                                            ...
                                            <div class="w-max">
                                                <a href="{{ route('admin.manage-users.show.basic-details', $user->id) }}"><x-icon.chevron-right
                                                        class="w-full" /></a>
                                            </div>
                                        </div>
    ...
    <div class="w-full flex items-center justify-center gap-8">
        @if ($users->hasPages())
            <div class="flex flex-row items-center h-10 gap-0 mt-4 w-max border rounded-md overflow-hidden">
                <button wire:click="previousPage" {{ $users->onFirstPage() ? 'disabled' : '' }}
                    @click="$wire.set('checkedUsers', [])"
                ...
                @foreach ($elements as $element)
                    ...
                    @endif
                @endforeach
                <button wire:click="nextPage" {{ !$users->hasMorePages() ? 'disabled' : '' }}
                    @click="$wire.set('checkedUsers', [])"
        ...
        <x-modal.confirmation-modal title="Confirmation">
            ...
            <x-slot:action_buttons>
                ...
                <template x-if="groupConfirmationModal.actionType === 'activate'">
                    <x-button.filled-button wire:target='multipleActivate, multipleDeactivate'
                        wire:loading.attr='disabled' wire:loading.class='cursor-progress'
    ...
    <x-loader.black-screen wire:loading wire:target="activeBox" class="z-10" />
</x-main.content>
