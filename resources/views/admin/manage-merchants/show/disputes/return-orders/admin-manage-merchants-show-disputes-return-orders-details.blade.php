<x-main.content class="!px-16 !py-10">
                ...
                <div class="flex flex-col justify-center w-4/12 p-4">
                    <div class="flex flex-col gap-2">
                        <p class="text-rp-neutral-500">Paid Price + Shipping Fee: <span class="text-rp-neutral-700">
                                {{ \Number::currency($return_order->product_order->amount * $return_order->product_order->quantity + $return_order->product_order->shipping_fee, 'PHP') }}</span>
                        </p>
                        <p class="text-rp-neutral-500">
                            ...
                            <span class="text-rp-neutral-700">₱
                                @if (in_array($return_order->status->name, ['Refunded Only', 'Returned and Refunded']))
                                    {{ \Number::currency($return_order->product_order->amount * $return_order->product_order->quantity + $return_order->product_order->shipping_fee, 'PHP') }}
                                @else
                                    0.00
                                @endif
                            </span>
        ...
        @if ($return_order->status->name === 'Pending Resolution')
            <div>
                ...
                <div class="flex flex-row gap-2">
                    <x-button.primary-gradient-button wire:click="$set('action','refund_only')"
                        class="w-[162px] py-2.5">Refund Only</x-button.primary-gradient-button>
                    <x-button.primary-gradient-button wire:click="$set('action','return_only')"
                        class="w-[162px] py-2.5">Return Only</x-button.primary-gradient-button>
                    <x-button.primary-gradient-button wire:click="$set('action','return_and_refund')"
                        class="w-[162px] py-2.5">Return & Refund</x-button.primary-gradient-button>
                    <x-button.primary-gradient-button wire:click="$set('action','cancel')"
                        class="w-[162px] py-2.5">Cancel</x-button.primary-gradient-button>
                </div>
            </div>
        @endif
    </div>
    ...
    @if ($action)
        @switch($action)
            @case('refund_only')
                <livewire:admin.disputes.return-orders.modals.admin-return-orders-refund-modal :merchant="$merchant"
                    :return_order_id="$return_order->id" />
            @break

            @case('return_only')
                <livewire:admin.disputes.return-orders.modals.admin-return-orders-return-modal :merchant="$merchant"
                    :return_order_id="$return_order->id" />
            @break

            @case('return_and_refund')
                <livewire:admin.disputes.return-orders.modals.admin-return-orders-return-refund-modal :merchant="$merchant"
                    :return_order_id="$return_order->id" />
            @break

            @case('cancel')
                <livewire:admin.disputes.return-orders.modals.admin-return-orders-cancel-modal :merchant="$merchant"
                    :return_order_id="$return_order->id" />
            @break
        @endswitch
    @endif
