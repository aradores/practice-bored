    ...
    <x-layout.details.more-details class="mb-8">
        ...
        <x-layout.details.more-details.section title="General Dispute Details" title_text_color="primary">
            <div class="space-y-2">
                ...
                <x-layout.details.more-details.data-field field="Transaction Date" value="{{ \Carbon\Carbon::parse($dispute->transaction->created_at)->timezone('Asia/Manila')->format('m/d/Y - h:i A') }}" />                
                <x-layout.details.more-details.data-field field="Transaction Amount" value="{{ \Number::currency($dispute->transaction->amount, 'PHP') }}" />
                <x-layout.details.more-details.data-field field="Service Fee" value="{{ \Number::currency($dispute->transaction->service_fee + $dispute->transaction->repay_cut, 'PHP') }}" />
                <x-layout.details.more-details.data-field field="Transaction Number" value="{{ $dispute->transaction->txn_no }}" />
                <x-layout.details.more-details.data-field field="Transaction Reference Number" value="{{ $dispute->transaction->ref_no }}" />
                <x-layout.details.more-details.data-field field="Date Created" value="{{ \Carbon\Carbon::parse($dispute->created_at)->timezone('Asia/Manila')->format('m/d/Y - h:i A') }}" />
            </div>
        </x-layout.details.more-details.section>
