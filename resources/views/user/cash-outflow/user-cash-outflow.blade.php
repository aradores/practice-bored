<div class="flex h-full">
    <x-main.content class="overflow-y-auto grow {{ $transactionDetails ? '!px-[18.5px]' : '' }}">
        <x-main.action-header>
            ...
            <x-slot:actions>
                <div class="flex flex-row items-center gap-3">
                    ...
                    @if ($color === 'red')
                        <x-button.filled-button href="{{ route('user.cash-outflow.create') }}">add outflow
                            transaction</x-button.filled-button>
                    @else
                        <x-button.filled-button href="{{ route('admin.transactions.cash-outflow.create') }}"
                            color="primary">add outflow transaction</x-button.filled-button>
                    @endif
                </div>
            </x-slot:actions>
        </x-main.action-header>
        ...
        <div class="relative grid {{ $color === 'red' ? 'grid-cols-5' : 'grid-cols-4' }} gap-[15px] mb-8">
            <x-card.filter-card wire:click="handleFilterBoxClick('')" :isActive="$activeBox === ''" label="All" :data="$allCashOutflow"
                :color="$color" />

            <x-card.filter-card wire:click="handleFilterBoxClick('transfer')" :isActive="$activeBox === 'transfer'" label="Money Transfers"
                subtitle="{{ Number::currency($moneyTransfersTotal, 'PHP') }}" :data="$moneyTransfers" :color="$color" />

            <x-card.filter-card wire:click="handleFilterBoxClick('order_payment')" :isActive="$activeBox === 'order_payment'"
                subtitle="{{ Number::currency($orderPaymentsTotal, 'PHP') }}" label="Order Payments" :data="$orderPayments" :color="$color" />

            <x-card.filter-card wire:click="handleFilterBoxClick('cash_out')" :isActive="$activeBox === 'cash_out'" label="Cash Out"
                subtitle="{{ Number::currency($cashOutflowTotal, 'PHP') }}" :data="$cashOutflowCount" :color="$color" />
            @if ($color === 'red')
                <a href="{{ route('user.bills') }}"
                    ...
                    <span class="text-xl font-bold">{{ Number::currency($billsTotal, 'PHP') }}</span>
                </a>
            @endif
        </div>
        ...
        <div class="space-y-8">
            ...
            <div class="overflow-auto">
                <x-table.rounded>
                    <x-slot:table_header>
                        <x-table.rounded.th
                            class="{{ $transactionDetails ? '' : 'w-52 min-w-52 max-w-52' }}">Transaction
                            type</x-table.rounded.th>
                        <x-table.rounded.th
                            class="{{ $transactionDetails ? '' : 'w-72 min-w-72 max-w-72' }}">Recipient</x-table.rounded.th>
                    ...
                    <x-slot:table_data>
                        ...
                        @foreach ($cashOutflows as $key => $cashOut)
                            ...
                            <x-table.rounded.row
                                ...
                                <x-table.rounded.td>
                                    {{ $cashOut->recipientName }}
                                </x-table.rounded.td>
                                <x-table.rounded.td
                                    class="{{ $color === 'red' ? 'text-rp-red-500' : 'text-primary-600' }} font-bold">
    ...
    @endif
    <x-loader.black-screen wire:loading.delay wire:target="dateFilter,handleFilterBoxClick,handleTableRowClick">
        <x-loader.clock />
