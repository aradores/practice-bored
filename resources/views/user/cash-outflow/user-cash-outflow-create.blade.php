<div class="flex h-full" x-data="timerData">
    <x-main.content class="overflow-y-auto grow !px-[18.5px]">
        ...
        <x-card.display-balance :balance="$this->available_balance" class="mb-8" :color="$color" />
        ...
        @if ($transaction_type === 'money-transfer')
            <div class="mb-3">
                <x-input.input-group>
                    ...
                    <x-dropdown.select wire:model.live="transfer_to" id="transfer_to"
                        class="{{ $errors->has('amount') ? '!border-red-500' : '' }}">
                ...
                <div class="mb-3">
                    <div class="flex flex-row justify-between gap-2">
                        <x-input.input-group class="flex-1">
                            <x-slot:label for="phone_number"
                                class="{{ $errors->has('amount') ? '!text-red-500' : '' }}">Phone Number:</x-slot:label>
                            <div class="flex flex-row gap-2">
                                <x-dropdown.select wire:model.change="phone_iso"
                                    class="w-32 {{ $errors->has('phone_iso') ? '!border-red-500' : '' }}">
                                ...
                                <x-input type="text" id="phone_number" wire:model.blur="phone_number" maxlength="15"
                                    class="w-full {{ $errors->has('phone_number') ? '!border-red-500' : '' }}"
                        ...
                        <x-input.input-group class="flex-1">
                            <x-slot:label for="amount"
                                class="{{ $errors->has('amount') ? '!text-red-500' : '' }}">Amount:</x-slot:label>
                            <x-input wire:model.blur="amount" type="text" maxlength="5"
                                class="{{ $errors->has('amount') ? '!border-red-500' : '' }}"
                                oninput="this.value = this.value.replace(/[^0-9]/gi, '')">
                            </x-input>
                        </x-input.input-group>
            ...
            @elseif ($transfer_to === 'another-bank')
                ...
                <x-input.input-group class="mb-3">
                    ...
                    @if ($send_via === 'instapay')
                        ...
                        @if (empty($this->instapay_banks_list))
                            <p class="text-red-500">Instapay merchants are unavailable at the moment</p>
                        @endif
                    @elseif ($send_via === 'pesonet')
                        ...
                        @if (empty($this->pesonet_banks_list))
                            <p class="text-red-500">Pesonet merchants are unavailable at the moment</p>
                        @endif
                    @endif
                    @error('selected_bank')
                        <p class="text-xs text-red-500">{{ $message }}</p>
        ...
        @elseif ($transaction_type === 'bill-payment')
            <div class="mb-3" x-data="{ show_billers: true }" @click="show_billers=true"
                @click.outside="show_billers=false">
                <x-input.input-group>
                    <x-slot:label for="biller_type">Biller type:</x-slot:label>
                    <x-input type="search" id="biller_type" wire:model.live="biller_type"
                        placeholder="Search biller">
                    </x-input>
                    <div class="overflow-y-auto max-h-64 py-2" x-show="show_billers">
                        @if (!empty($this->billers_list))
                            @foreach ($this->show_billers_list as $key => $biller_opt)
                                <button
                                    class="w-full items-center text-left justify-between border border-black rounded-lg px-5 py-4 mb-2 {{ $biller_opt['status'] == 0 ? 'cursor-not-allowed bg-gray-200' : 'bg-white hover:bg-gray-100' }}"
                                    wire:key="biller-{{ $biller_opt['code'] }}"
                                    {{ $biller_opt['status'] == 0 ? 'disabled' : '' }}
                                    wire:click="$set('biller_type', '{{ $biller_opt['name'] }}')">
                                    <p>{{ $biller_opt['name'] }}</p>
                                </button>
                            @endforeach
                        @else
                            <p class="text-red-500">Billers are unavailable at the moment</p>
                        @endif
                    </div>
                    @error('biller_type')
                        <p class="text-xs text-red-500">{{ $message }}</p>
            ...
            <div class="flex flex-row justify-between gap-2 mb-3">
                @foreach ($this->get_biller_input_fields as $key => $input_field)
                    <x-input.input-group class="flex-1" wire:key="input-field-{{ $key }}">
                        <x-slot:label
                            for="input-field-{{ $key }}">{{ $input_field['label'] }}:</x-slot:label>
                        <x-input type="text" maxlength="{{ $input_field['maxlength'] }}"
                            wire:model.blur="bill_info.{{ $key }}" id="input-field-{{ $key }}">
                        </x-input>
                        @error('bill_info.' . $key)
                            <p class="text-xs text-red-500">{{ $message }}</p>
                        @enderror
    ...
    <x-layout.summary>
        ...
        <x-slot:body>
            @if ($transaction_type === 'money-transfer')
                @if ($transfer_to === 'another-account')
                    <x-layout.summary.section title="Payment Details" :color="$color">
                        <x-slot:data>
                    ...
                    @if (!empty($message))
                        <x-layout.summary.section title="Message" :color="$color">
                            <x-slot:data>
                                <p>{{ $message }}</p>
                            </x-slot:data>
                        </x-layout.summary.section>
                ...
                @elseif ($transfer_to === 'unionbank-account')
                    ...
                    <x-layout.summary.section title="Notification Details" :color="$color">
                        <x-slot:data>
                            <x-layout.summary.label-data>
                                <x-slot:label>Phone Number</x-slot:label>
                    ...
                    <x-slot:info_block>
                        @if (!empty($biller_type))
                            <div class="gap-2 text-center">
                                <p class="font-bold text-rp-neutral-700 text-xl text-balance">
                                    {{ $this->get_biller_name }}</p>
                    ...
                    <x-slot:data>
                        ...
                        @if (!empty($amount))
                            ...
                            <x-layout.summary.label-data>
                                <x-slot:label>Total</x-slot:label>
                                <x-slot:data
                                    class="{{ $is_admin ? 'text-primary-600' : 'text-rp-red-500' }}">
        ...
        <x-slot:action>
            ...
            <div class="flex flex-col gap-3">
                <x-button.filled-button @click="onSubmit" :color="$color" :disabled="$this->button_submit_clickable == false"
                    form="cash-outflow-form" size="md">send</x-button.filled-button>
                @if (isset($merchant))
                    <x-button.outline-button
                        href="{{ $is_admin
                            ? route('admin.transactions.cash-outflow.index')
                            : route('merchant.financial-transactions.cash-outflow.index', ['merchant' => $merchant]) }}"
                        size="md" :color="$color">cancel</x-button.outline-button>
                @else
                    <x-button.outline-button
                        href="{{ $transaction_type === 'bill-payment' ? route('user.bills') : route('user.cash-outflow.index') }}"
                        size="md" :color="$color">cancel</x-button.outline-button>
                @endif
            </div>
        </x-slot:action>
    ...
    <x-modal x-model="$wire.show_otp_modal">
        <x-modal.form-modal title="OTP Verification">
            <x-input.input-group>
                <x-slot:label>We sent an OTP verification code to your number. Enter the code here to
                    continue.</x-slot:label>
                <div class="w-full flex flex-row gap-2">
                    <x-input class="flex-1" type="text" wire:model='otp' maxlength="6"
                        oninput="this.value = this.value.replace(/[^0-9]/gi, '')" />
            ...
            <x-slot:action_buttons>
                <x-button.outline-button wire:click="$dispatch('closeModal')" wire:target='submit' :color="$color"
                    wire:loading.attr='disabled' class="w-1/2">cancel</x-button.outline-button>
                <x-button.filled-button wire:click='submit' wire:target='submit' wire:loading.attr='disabled'
                    :color="$color" wire:loading.class='cursor-progress'
                    class="w-1/2">confirm</x-button.filled-button>
            </x-slot:actions>
        </x-modal.form-modal>
    </x-modal>
    ...
    <x-loader.black-screen wire:loading.delay.longer
        wire:target="transaction_type,transfer_to,submit,get_biller_input_fields,cancel_otp_verification,verify_otp">
        <x-loader.clock />
    ...
    {{-- Success message --}}
    <div>
        <div x-cloak x-show="$wire.apiSuccessMsg" @click="$wire.apiSuccessMsg=false"
