<x-main.content x-data="{
        ...
        @foreach ($orders as $key => $order)
            <div role="button" tabindex="0" @keyup.enter="$wire.open_order_show({{$order->id}})" wire:click.stop="open_order_show('{{ $order->order_number }}')"
                ...
                <div class="flex flex-row py-3 justify-between items-center break-words">
                    ...
                    {{-- Total --}}
                    <div class="flex flex-col text-rp-neutral-700 justify-center w-3/12 break-words px-2">
                        <p>Total Product Price: {{ \Number::currency(($order->amount * $order->quantity), 'PHP') }}</p>
                        <p>Shipping Fee: {{ \Number::currency($order->shipping_fee, 'PHP') }}</p>
                        <p class="font-bold">Total: {{ \Number::currency(($order->amount * $order->quantity) + $order->shipping_fee, 'PHP') }}</p>
                    </div>
                    {{-- Status --}}
                    ...
                    <div class="flex flex-col gap-3 justify-center w-2/12 px-2">
                        @switch($order->shipping_status->name)
                            @case('Pending')
                            @case('Packed')
                            {{-- TODO: CANCEL ORDER FOR READY TO SHIP --}}
                            @case('Ready to Ship')
                            @case('To Ship')
    ...
    {{-- CANCEL ORDER MODAL --}}
    @if ($order_cancel_id)
        <x-modal x-model="$wire.order_cancel_id">
