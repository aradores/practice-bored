<x-main.content x-data>
    <x-main.action-header>
        ...
        <x-slot:actions>
            @switch($product_order->shipping_status->name)
                ...
                @case('Ready to Ship')
                @case('To Ship')
                    <x-button.filled-button @click="$wire.show_cancel_order_modal=true" size="md">cancel</x-button.filled-button>
                @break
    ...
    {{-- ------------ MODAL STARTS HERE -------------- --}}
    {{-- CANCEL ORDER --}}
    @if (in_array($product_order->shipping_status_id, $accepted_status))
        <template x-teleport="body">
            <x-modal x-model="$wire.show_cancel_order_modal">
                <livewire:user.orders.modals.user-cancel-order-modal :order_id="$product_order->id" />
            </x-modal>
        </template>
    @endif
