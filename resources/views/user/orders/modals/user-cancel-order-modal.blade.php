<x-modal.form-modal title="Cancel Order">
        ...
        <x-input.input-group class="mb-6">
            ...
            <x-dropdown.select wire:model='reason'>
                ...
                @foreach ($this->get_cancel_reasons as $key => $reason_option)
                    <x-dropdown.select.option value="{{ $reason_option->slug }}"
                        wire:key="reason-{{ $key }}">{{ $reason_option->name }}</x-dropdown.select.option>
                @endforeach
            </x-dropdown.select>
            @error('reason')
    ...
    <x-slot:action_buttons>
        <x-button.outline-button wire:click="$dispatch('closeModal')" class="w-1/2">cancel</x-button.outline-button>
        <x-button.filled-button wire:click='submit' wire:target='submit' wire:loading.attr='disabled' :disabled="$button_clickable == false"
