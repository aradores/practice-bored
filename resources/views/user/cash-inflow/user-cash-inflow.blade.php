<div class="h-full flex">
    <x-main.content class="overflow-y-auto grow {{ $transactionDetails ? '!px-[18.5px]' : '' }}">
        ...
        <div class="relative grid grid-cols-4 gap-[15px] mb-8">
            <x-card.filter-card wire:click="handleFilterBoxClick('')" :isActive="$activeBox === ''" label="All" :data="$allCashInflowCount"
                :color="$color" />
            <x-card.filter-card wire:click="handleFilterBoxClick('transfer')" :isActive="$activeBox === 'transfer'" label="Money Transfers"
                subtitle="{{ Number::currency($moneyTransferTotal, 'PHP') }}" :data="$moneyTransferCount" :color="$color" />

            <x-card.filter-card wire:click="handleFilterBoxClick('cash_in')" :isActive="$activeBox === 'cash_in'" label="Cash In"
                subtitle="{{ Number::currency($cashInTotal, 'PHP') }}" :data="$cashInCount" :color="$color" />
            <x-card.filter-card wire:click="handleFilterBoxClick('payroll_salary')" :isActive="$activeBox === 'payroll_salary'"
                subtitle="{{ Number::currency($salaryTotal, 'PHP') }}" label="Salary" :data="$salaryCount"
                :color="$color" />
        </div>
            ...
            <div class="overflow-auto">
                <x-table.rounded>
                    <x-slot:table_header>
                        ...
                        <x-table.rounded.th
                            class="{{ $transactionDetails ? '' : 'w-72 min-w-72 max-w-72' }}">Sender</x-table.rounded.th>
                        ...
                        @foreach ($cashInflows as $key => $cashIn)
                            ...
                            @endphp
                                <x-table.rounded.row class="overflow-hidden cursor-pointer {{ $isActiveRow ? ($color === 'primary' ? 'border border-primary-600' : 'border border-rp-red-500') : 'hover:bg-rp-neutral-50' }}" wire:click="handleTableRowClick('{{ $cashIn->txn_no }}')">
                                    ...
                                    <x-table.rounded.td>
                                        @if ($cashIn->type->slug === 'cash_in' && isset($cashIn->extras['payment_channel']))
                                            {{ $cashIn->extras['payment_channel'] }}
                                        @else
                                    ...
                                    <x-table.rounded.td class="{{ $color === 'primary' ? 'text-primary-600' : 'text-rp-red-500' }} font-bold">
                                        {{ \Number::currency($cashIn->amount, $cashIn->currency) }}
                                    </x-table.rounded.td>
                                    <x-table.rounded.td>
