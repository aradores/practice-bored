<header class="px-5 py-2 h-[60px] flex flex-row justify-end items-center bg-white " x-data="{
    ...
    <div class="relative">
        <button @click="isDropdownVisible=!isDropdownVisible" class="flex flex-row items-center gap-3 cursor-pointer">
            <div class="flex flex-row items-center gap-2">
                @if ($profile_picture = auth()->user()->getMedia('profile_picture')->first())
                    <img src="{{ $profile_picture->disk === 's3' ? $profile_picture->getTemporaryUrl(Carbon\Carbon::now()->addMinutes(5), 'thumbnail') : $profile_picture->getUrl('thumbnail') }}"
        ...
        <div x-cloak x-show="isDropdownVisible" @click.away="isDropdownVisible=false"
            ...
            @if (auth()->user()->roles()->where('slug', 'verified_user')->exists())
            ...
            @if (auth()->user()->hasAdminRole())
                <a href="{{ route('admin.index') }}"
                    ...
                    Dashboard</a>
            @endif
            <div role="button" tabindex="0" @keyup.enter="isLogoutModalVisible=true;"
                @click="isLogoutModalVisible=true"
                class="w-full px-3 py-2 text-left cursor-pointer hover:bg-rp-neutral-50 h-max">Logout</div>
        </div>
                        ...
                        @foreach ($employees as $key => $employee)
                            <div
                                ...
                                <div class="flex flex-col w-full">
                                    ...
                                    <h3 class="text-xl font-bold text-center truncate">{{ $employee->merchant->name }}
                                    </h3>
                                ...
                                @if ($employee->merchant->status == 'verified')
                                    ...
                                    @endif
                                @elseif ($employee->merchant->status == 'pending')
                                    <p class="italic text-center text-stale-500">Merchant account is under review</p>
                                @else
                                    <p class="italic text-center text-stale-500">Merchant account is
                                        {{ $employee->merchant->status }}</p>
                                @endif
                            </div>
                        @endforeach
                    </x-slot:items>
        ...
        <x-modal.confirmation-modal title="Logout" message="Are you sure you want to log out?">
            <x-slot:action_buttons>
                <x-button.outline-button class="w-1/2"
                    @click="isLogoutModalVisible=false;">Cancel</x-button.outline-button>
                <form id="logout-form" method="POST" action="{{ route('logout') }}" class="hidden">
                    @csrf
                </form>
                <x-button.filled-button x-data @click.prevent="document.getElementById('logout-form').submit()"
                    class="w-1/2">Logout</x-button.filled-button>
            </x-slot:action_buttons>
