    ...
    <div class="bg-white py-6 rounded-lg w-full">
        <div class="w-[80%] mx-auto">
            ...
            <div class="flex flex-col gap-2">
                @if (isset($transactionDetails['extras']))
                    @foreach ($transactionDetails['extras'] as $label => $value)
                ...
                @if (isset($transactionDetails['repay_cut']) && $transactionDetails['repay_cut'] > 0)
                    <div class="flex flex-row justify-between">
                        <span>Repay Fee</span>
                        <span>{{ \Number::currency($transactionDetails['repay_cut'], 'PHP') }}</span>
                    </div>
                @endif
    
                <div class="flex flex-row justify-between">
                    <span class="font-bold">Total</span>
                    <span class="text-lg font-bold">{{ isset($transactionDetails['total_amount']) ? \Number::currency($transactionDetails['total_amount'], 'PHP') : \Number::currency($transactionDetails['amount'], 'PHP') }}</span>
                </div>
            </div>
