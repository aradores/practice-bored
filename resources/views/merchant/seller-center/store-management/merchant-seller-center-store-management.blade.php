<x-main.content x-data="{ featuredServices: false, featuredProducts: false, showStockRange: false, showPriceRange: false }">
    @vite(['resources/js/swiper-store-management-cards.js'])
    ...
    <div class="h-[540px]">
        ...
        @else
            @php
                $src = $merchant_details->getFirstMediaUrl('merchant_banner')
                    ? $this->get_media_url($merchant_details->getFirstMedia('merchant_banner'))
                    : null;
            @endphp
        @endif
        <div class="h-[414px] bg-no-repeat place-items-end grid bg-cover bg-center rounded-lg"
            ...
            @if ($can_update)
                ...
                <input type="file" name="banner_src" id="banner_src" class="hidden" wire:model="banner_file"
                    accept="image/*">
        ...
        <div class="-translate-y-6 px-[23px] flex  justify-between">
            <div class="flex ">
                ...
                <div class="relative">
                    ...
                    @if ($can_update)
                        <input type="file" name="photo_src" id="photo_src" class="hidden" wire:model="photo_file"
                            accept="image/*">
                ...
                <div class="flex flex-col w-64 2xl:w-72">
                    ...
                    <div class="flex w-full">
                        ...
                        <div class="w-1/2 text-rp-purple-400 text-end">
                            <p class="text-end text-rp-purple-400">
                                {{ $merchant->created_at->timezone('Asia/Manila')->format('F j, Y') }}
                            </p>
                        </div>
                    </div>
                </div>
    ...
    {{-- STORE ABOUT SECTION --}}
    <livewire:merchant.seller-center.store-management.store-management-about-store :merchant="$merchant" :can_update="$can_update"
        :key="'about-store-' . '{{ $merchant->id }}'" />

    {{-- Featured Products --}}
    <livewire:merchant.seller-center.store-management.store-management-featured-products :merchant="$merchant"
        :can_update="$can_update" :key="'featured-products-' . '{{ $merchant->id }}'" />

    {{-- Featured Services --}}
    <livewire:merchant.seller-center.store-management.store-management-featured-services :merchant="$merchant"
        :can_update="$can_update" :key="'featured-services-' . '{{ $merchant->id }}'" />
                ...
                <div class="w-[215px] mb-4">
                        ...
                        <div class="relative w-[215px] h-full">
                            @if ($product->first_image)
                                <img class="object-cover w-full h-full"
                                    src="{{ $this->get_media_url($product->first_image, 'thumbnail') }}"
                                    alt="">
                            @endif
                    ...
                    <div class=" p-[13px] bg-white rounded-b-[14px] shadow-md">
                        ...
                        <h2 class="text-rp-purple-600 text-[19px] font-bold">
                            {{ \Number::currency($product->price - $product->price * $product->sale_amount, 'PHP') }}
                        </h2>

                        @if ($product->sale_amount > 0)
                            <div class="flex gap-2">
                                {{-- PRODUCT ORIGINAL PRICE --}}
                                <h3 class="line-through text-rp-neutral-500">
                ...
                <div class="w-[215px] mb-4">
                    <div class=" h-[166px] rounded-t-[13px] overflow-hidden">
                        @if ($service->first_image)
                            <div class="relative w-[215px] h-full">
                                <img class="object-cover w-full h-full"
                                    src="{{ $this->get_media_url($service->first_image, 'thumbnail') }}"
                                    alt="">
                            </div>
                        @endif
                    </div>
