<div class="absolute flex flex-col gap-8 bg-white p-10 rounded-2xl w-[1180px] max-w-[90%] max-h-[95%] overflow-y-auto">
    ...
    {{-- HEADING --}}
    <div>
        <h3 class="text-2xl font-bold mb-2">Process Refund</h3>
            ...
            <td class="bg-white py-6 align-top">
                <div>
                    <h5 class="text-[13px] font-bold">
                        {{ \Number::currency($return_order->product_order->amount * $return_order->product_order->quantity, 'PHP') }}
                    </h5>
    ...
    <div class="grid grid-cols-2 gap-3">
        <x-button.outline-button @click="$dispatch('closeModal')" wire:target="process_refund" wire:loading.attr="disabled"
            wire:loading.class="opacity-50">Cancel</x-button.outline-button>
        <x-button.filled-button wire:click="process_refund" wire:target="process_refund" wire:loading.attr='disabled' :disabled="$button_clickable == false">Confirm</x-button.filled-button>
    </div>
    ...
    <x-loader.black-screen wire:loading.block wire:target="process_refund" class="z-10" />
</div>
