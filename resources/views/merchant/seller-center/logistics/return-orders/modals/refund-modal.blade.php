<div class="absolute flex flex-col gap-8 bg-white p-10 rounded-2xl w-[1180px] max-w-[90%] max-h-[95%] overflow-y-auto">
    ...
    <div>
        ...
        <p class="text-[13px]">This request will be chosen for direct refunds without return of product. Please
            confirm if you agree to refund the request.</p>
    </div>
        ...
        <tr>
            ...
            <td class="bg-white py-6 align-top">
                <div>
                    ...
                    <p class="text-[11px] w-fit px-2 py-[2px] text-rp-red-600 border border-rp-red-600 rounded-[5px]">{{ $return_order->product_order->payment_option->name }}</p>
                    <p class="text-[11px]">Paid Price + Shipping Fee: 
    ...
    <div class="grid grid-cols-2 gap-3">
        <x-button.outline-button @click="$dispatch('closeModal')" wire:target="refund_order" wire:loading.attr="disabled"
            wire:loading.class="opacity-50">Cancel</x-button.outline-button>
        <x-button.filled-button wire:click="refund_order" wire:target="refund_order" wire:loading.attr='disabled' :disabled="$button_clickable == false">Confirm</x-button.filled-button>
    </div>
    ...
    <x-loader.black-screen wire:loading.block wire:target="refund_order" class="z-10" />
</div>
