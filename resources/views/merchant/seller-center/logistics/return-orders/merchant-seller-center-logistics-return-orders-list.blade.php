                    ...
                    <x-table.rounded.row @click="$wire.view_return_order({{ $return_order->id }})" class="cursor-pointer hover:bg-rp-neutral-50">
                        ...
                        <x-table.rounded.td>
                            <div class="text-rp-neutral-600">
                                ...
                                <p class="text-[11px]">Paid Price + Shipping Fee: {{ \Number::currency(($return_order->product_order->amount * $return_order->product_order->quantity) + $return_order->product_order->shipping_fee, 'PHP') }}</p>
                                @if (in_array($return_order->status->slug, ['refunded_only', 'returned_and_refunded']))
                                    <p class="text-[11px]">Refund Amount: {{ \Number::currency(($return_order->product_order->amount * $return_order->product_order->quantity) + $return_order->product_order->shipping_fee, 'PHP') }}</p>
                                @else
