<header class="w-full h-[60px] px-5 py-2 flex flex-row justify-between items-center bg-white" x-data="{
        ...
        <div class="flex flex-row items-center gap-3">
            ...
            <button @click="isDropdownVisible=!isDropdownVisible" class="flex flex-row items-center gap-5 cursor-pointer" wire:ignore>
                <div class="pl-3 border-l-2 border-l-gray-700 border-3">
                    <div class="flex flex-row items-center gap-3">
                        ...
                        @endphp                        
                        <img src="{{ $merchant_logo }}" class="object-cover w-11 h-11 rounded-full" alt="">
                    </div>
                </div>
                <div>
            ...
            <div x-cloak x-show="isDropdownVisible" @click.away="isDropdownVisible=false"
                ...
                @can('merchant-cash-inflow', [request('merchant'), 'view'])
                    <a  href="{{ route('merchant.financial-transactions.cash-inflow', ['merchant' => request('merchant')]) }}"
                ...
                @can('merchant-cash-outflow', [request('merchant'), 'view'])
                    <a  href="{{ route('merchant.financial-transactions.cash-outflow.index', ['merchant' => request('merchant')]) }}"
                ...
                @can('merchant-invoices', [request('merchant'), 'view'])
                    <a  href="{{ route("merchant.financial-transactions.invoices.index", ['merchant' => request('merchant')]) }}"
                    class="w-full px-3 py-2 text-left cursor-pointer hover:bg-rp-neutral-50 h-max">Invoices</a>
                @endcan
                @can('merchant-bills', [request('merchant'), 'view'])
                    <a  href="{{ route("merchant.financial-transactions.bills.index", ['merchant' => request('merchant')]) }}"
                ...
                @can('merchant-employees', [request('merchant'), 'view'])
                    <a  href="{{ route('merchant.financial-transactions.employees.index', ['merchant' => request('merchant')]) }}"
                    class="w-full px-3 py-2 text-left cursor-pointer hover:bg-rp-neutral-50 h-max">Employees</a>
                @endcan
                @can('merchant-payroll', [request('merchant'), 'view'])
                    <a  href="{{ route('merchant.financial-transactions.payroll.index', ['merchant' => request('merchant')]) }}"
                    class="w-full px-3 py-2 text-left cursor-pointer hover:bg-rp-neutral-50 h-max">Payroll</a>
                @endcan
