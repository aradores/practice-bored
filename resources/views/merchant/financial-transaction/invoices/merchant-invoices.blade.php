<div class="flex h-full">
                ...
                <div class="flex flex-row items-center gap-3">
                    ...
                    <div wire:ignore class="flex gap-1">
                        @if ($color == 'primary')
                            ...
                            <x-button.outline-button color="primary" href="{{ route('admin.transactions.invoices.bulk-uploads.index') }}">
                                bulk invoice
                        ...
                        @else
                            @can('merchant-invoices', [$merchant, 'create'])
                                ...
                                <x-button.outline-button
                                    href="{{ route('merchant.financial-transactions.invoices.bulk-uploads.index', ['merchant' => $merchant->account_number]) }}">bulk
                                    invoice</x-button.outline-button>
                            @endcan
                        @endif
                    </div>
                </div>
                            ...
                            <x-table.rounded.row
                                ...
                                <x-table.rounded.td>
                                    @switch($invoice->status)
                                        ...
                                        @case('unpaid')
                                            @if (\Carbon\Carbon::parse($invoice->due_date)->timezone('Asia/Manila') < \Carbon\Carbon::now('Asia/Manila')->startOfDay())
    ...
    @if (!is_null($invoice_data))
        <div class="min-w-[35%] max-w-[35%] h-full px-5 py-10 overflow-auto grow">
            <div class="w-full">
    ...
    @endif
    <x-loader.black-screen wire:loading.delay.longer wire:target="dateFilter,previousPage,nextPage,gotoPage,show_invoice,handleFilterBoxClick">
        <x-loader.clock />
