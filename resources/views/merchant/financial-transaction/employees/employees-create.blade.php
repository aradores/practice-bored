<div class="flex h-full" x-data="{
    ...
    <x-main.content class="flex-1 h-full overflow-auto !px-[18.5px]">
        ...
        <div x-cloak x-show="$wire.mode === 'no_repay_account'" class="flex flex-col gap-9">
            ...
            <div>
                ...
                <div class="grid xl:grid-cols-2 2xl:grid-cols-3 gap-3">
                    {{-- Phone number --}}
            ...
            <div>
                ...
                <div class="grid xl:grid-cols-2 2xl:grid-cols-4 gap-3 mb-3">
                    {{-- First name --}}
                    <x-input.input-group>
                    ...
                    <div class="flex flex-col basis-1">
                        ...
                        <div class="flex flex-row gap-1 2xl:gap-2">
                            ...
                            <x-input wire:model.blur="phone_number" id="phone_number" type="text"
                                placeholder="9123456789" class="grow" maxlength="10"
                                oninput="this.value = this.value.replace(/[^0-9]/g, '')"
                                x-bind:disabled="$wire.phone_iso === ''">
                                <x-slot:icon>
                                    <x-icon.phone />
    ...
    <x-layout.summary>
        ...
        <x-slot:body>
            <x-layout.summary.section title="Contact Information" :color="$color">
                <x-slot:data>
            ...
            <template x-if="$wire.mode === 'no_repay_account'">
                <x-layout.summary.section title="Personal Information" :color="$color">
                    <x-slot:data>
                        <x-layout.summary.label-data>
                            <x-slot:label>
                                Full Name
            ...
            <x-layout.summary.section title="Employee Details" :color="$color">
                <x-slot:data>
                    <x-layout.summary.label-data>
                        <x-slot:label>
                            Position
        ...
        <x-slot:action>
            ...
            @if ($mode === 'no_repay_account')
                <div class="flex flex-col gap-3">
                    <x-button.filled-button wire:click="add_employee_no_repay_account" :color="$color"
                        x-bind:disabled="$wire.isAgree === false" size="lg">send</x-button.filled-button>

                    @if ($color == 'red')
                        <x-button.outline-button size="lg" :color="$color"
                    ...
                    @else
                        <x-button.outline-button size="lg" :color="$color"
                            href="{{ route('admin.employees.create') }}">cancel</x-button.outline-button>
                    @endif
                </div>
            @elseif ($mode === 'has_repay_account')
                <div class="flex flex-col gap-3">
                    @if ($notified)
                        <x-button.filled-button @click="$wire.isEmployeeResendModalVisible=true" x-bind:disabled="$wire.isAgree === false" :color="$color"
                            size="lg">send</x-button.filled-button>
                    @else
                        <x-button.filled-button wire:click="add_employee_has_repay_account" x-bind:disabled="$wire.isAgree === false" :color="$color"
                            size="lg">send</x-button.filled-button>
                    @endif
                ...
                <x-modal x-model="$wire.isEmployeeResendModalVisible">
                    <x-modal.confirmation-modal>
                        ...
                        <x-slot:action_buttons>
                            <x-button.outline-button class="flex-1" :color="$color"
                                @click="$wire.isEmployeeResendModalVisible=false;">cancel</x-button.outline-button>
                            <x-button.filled-button class="flex-1" :color="$color" wire:click="add_employee_has_repay_account;">proceed</x-button.filled-button>
                        </x-slot:action_buttons>
                    </x-modal.confirmation-modal>
                </x-modal>
            @endif
