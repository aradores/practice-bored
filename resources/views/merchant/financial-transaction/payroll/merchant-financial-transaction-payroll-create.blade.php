<div class="flex h-full" x-data>
    <x-main.content class="grow overflow-auto !px-[18.5px]">
        ...
        <x-card.display-balance :balance="$this->available_balance" :color="$color" class="mb-8" />
        ...
        <div class="space-y-4">
            ...
            {{-- Employee Selection --}}
            <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-2 w-full min-h-[270px]">
                {{-- Go to Employees --}}
                <a href="{{ $employees_page_url }}"
                ...
                @foreach ($employees as $employee)
                    <div role="button" tabindex="0" @keyup.enter="$wire.handleEmployeeSelect({{ $employee->id }})"
                        wire:key="employee-{{ $employee->id }}" wire:click="handleEmployeeSelect({{ $employee->id }})"
                        ...
                        <p class="font-bold text-xl text-center w-full wrap-text">{{ $employee->user->name }}</p>
                        <p class="text-sm text-center wrap-text w-full">{{ $employee->occupation }}</p>
                        <p class="text-center text-sm wrap-text w-full">Salary: <span
                                class="{{ $is_admin ? 'text-primary-600' : 'text-rp-red-500' }}">{{ Number::currency($employee->salary, 'PHP') }}</span>
                        </p>
                        <p class="text-center text-sm wrap-text w-full">Salary Type: <span
                                class="{{ $is_admin ? 'text-primary-600' : 'text-rp-red-500' }}">{{ $employee->salary_type->name }}</span>
                        </p>
                    </div>
                @endforeach
            </div>
    ...
    <x-layout.summary>
        @if ($selectedEmployee)
            ...
            <x-slot:body>
                <x-layout.summary.section title="Salary Details" :color="$color">
                    <x-slot:data>
                        <x-layout.summary.label-data>
                            <x-slot:label>Amount</x-slot:label>
                            <x-slot:data>{{ \Number::currency($selectedEmployee->salary, 'PHP') }}</x-slot:data>
                        </x-layout.summary.label-data>
                        ...
                        <x-layout.summary.label-data>
                            <x-slot:label>Deductions</x-slot:label>
                            <x-slot:data>{{ \Number::currency($deductions * -1, 'PHP') }}</x-slot:data>
                        </x-layout.summary.label-data>
                        ...
                        <x-layout.summary.label-data>
                            ...
                            <x-slot:data
                                class="{{ $is_admin ? 'text-primary-600' : 'text-rp-red-500' }}">{{ \Number::currency($net_pay, 'PHP') }}</x-slot:data>
                        </x-layout.summary.label-data>
            ...
            <x-slot:action>
                <div class="flex flex-col gap-2">
                    ...
                    <x-button.filled-button wire:click='showConfirmationModal' :disabled="$salaryDetailErrorMessage"
                        wire:target='showConfirmationModal' wire:loading.attr='disabled'
                        wire:loading.class='cursor-progress' :color="$color">
                        send
                    </x-button.filled-button>
                    <x-button.outline-button href="{{ $payroll_page_url }}" :color="$color">
                        cancel
        ...
        @else
                ...
                <x-layout.summary.section title="Salary Details" :color="$color">
                    <x-slot:data>
                        ...
                        <x-layout.summary.label-data>
                            <x-slot:label>Net Pay</x-slot:label>
                            <x-slot:data
                                class="{{ $is_admin ? 'text-primary-600' : 'text-rp-red-500' }}">-</x-slot:data>
                        </x-layout.summary.label-data>
                    </x-slot:data>
                </x-layout.summary.section>
            </x-slot:body>
            <x-slot:action>
                <div class="flex flex-col gap-2">
                    <x-button.filled-button :disabled="true" :color="$color">
                        send
                    </x-button.filled-button>
                    <x-button.outline-button href="{{ $payroll_page_url }}" :color="$color">
                        cancel
        ...
        <x-modal.confirmation-modal title="Confirm Payroll?">
            ...
            <x-slot:action_buttons>
                <x-button.outline-button wire:target='submit' wire:loading.attr='disabled'
                    wire:loading.class='cursor-progress' @click="$wire.displayConfirmation=false;" class="flex-1"
                    :color="$color">go
                    back</x-button.outline-button>
                <x-button.filled-button wire:target='submit' wire:loading.attr='disabled'
                    wire:loading.class='cursor-progress' @click="$wire.submit()" :color="$color"
    ...
    <div x-cloak x-show="$wire.successMessage.length > 0"
        ...
        <div class="w-96 px-7 py-4 space-y-2 bg-white rounded-2xl">
            ...
            <p x-text="$wire.successMessage" class="font-semibold text-center text-sm"></p>
            <div class="mt-3">
                <template x-if="$wire.successAmount !== null">
                    <div class="flex flex-row justify-between py-2 border-b">
                        <p>Payment</p>
                        <p x-text="$wire.successAmount"></p>
                    </div>
                </template>
            ...
            <x-button.filled-button
                @click="$wire.successMessage='';$wire.successAmount=null;$wire.successRemainingBal=null"
                class="w-full" :color="$color">go back</x-button.filled-button>
        </div>
    </div>
    ...
    <div x-cloak x-show="$wire.failedMessage"
        ...
        <div class="w-96 px-14 py-4 space-y-2 bg-white rounded-2xl">
            ...
            <x-button.filled-button @click="$wire.failedMessage=''" class="w-full" :color="$color">go
