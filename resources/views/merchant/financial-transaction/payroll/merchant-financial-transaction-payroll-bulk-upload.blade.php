<x-main.content>
    ...
    <x-card.display-balance :balance="$this->balance_amount" :color="$color" />
    ...
    @empty($file_data)
        <div class="relative rounded-lg">
            <div class="relative flex flex-col items-center justify-center my-5 py-9">
                ...
                <x-button.filled-button :color="$color" class="mt-3" @keyup.enter="$refs.uploaded_csv_file.click();">
                    upload CSV file
                </x-button.filled-button>
                <input type="file" accept=".csv,.xlsx" class="absolute h-full w-full opacity-0 cursor-pointer"
        ...
        <div class="w-full my-6">
            <div class="flex flex-row justify-between">
                ...
                <div class="relative w-44 h-9">
                    <x-button.filled-button @click="$refs.uploaded_csv_file.click();" :color="$color">
                        replace csv file
                    </x-button.filled-button>
                    <input type="file" accept=".csv,.xlsx" class="absolute hidden opacity-0 cursor-pointer w-44 h-9"
                        wire:model="uploaded_csv_file" x-ref="uploaded_csv_file" id="uploaded_csv_file" />
                </div>
            </div>
                    ...
                    <tr>
                        @foreach ($headers as $key => $header)
                            <td
                                ...
                                {{ empty($row[$key]) ? ($key > 1 ? 0 : '') : (is_numeric($row[$key]) && $key != 1 && $key != 4 ? number_format($row[$key], 2) : $row[$key]) }}
                                {{-- if $key > 1 -> --}}
                            </td>
                        @endforeach
            ...
            <div class="flex flex-row gap-2 mt-5">
                <x-button.outline-button wire:click='cancel_upload' class="flex-1" :color="$color">
                    cancel
                </x-button.outline-button>
                <x-button.filled-button class="flex-1" :disabled="!$allowSubmit" @click="$wire.modal_visible=true" :color="$color">
                    Submit
                </x-button.filled-button>
            </div>
        </div>
            ...
            <x-modal.confirmation-modal>
                ...
                <x-slot:action_buttons>
                    <x-button.outline-button class="flex-1"
                        @click="$wire.modal_visible=false;" :color="$color">cancel</x-button.outline-button>
                    <x-button.filled-button class="flex-1" :disabled="!$allowSubmit" :color="$color"
    ...
    <div class="flex flex-row gap-2 mt-5">
        ...
        <p>IMPORTANT: RePay follows a very specific format for uploading excel files. Please use our formatted
            ...
            our format yet, you can <span role="button" tabindex="0" @keyup.enter="$wire.downloadFormat" class="underline font-semibold {{ $color == 'red' ? 'text-rp-dark-pink-600' : 'text-primary-600' }} cursor-pointer"
