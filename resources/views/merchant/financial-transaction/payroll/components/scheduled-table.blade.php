<x-table.standard class="overflow-auto max-w-full w-full">
    <x-slot:table_header class="text-rp-neutral-700 font-bold text-xs">
        ...
        <x-table.standard.th class="max-w-52">
            <div class="flex flex-row items-center gap-2">
                <div>Payout Date</div>
                @php
                    $payout_date_sort_direction = '';

                    if ($sort_by === 'payout_at') {
                        $payout_date_sort_direction = $sort_direction === 'desc' ? 'down' : 'up';
                    } else {
                        $payout_date_sort_direction = 'both';
                    }

                @endphp
                <button wire:click="changeSorting('payout_at')"><x-icon.sort :direction="$payout_date_sort_direction" height="26"
                        width="18" /></button>
            </div>
        </x-table.standard.th>
        ...
        <x-table.standard.th class="max-w-52">
            <div class="flex flex-row items-center gap-2">
                ...
                @endphp
                <button wire:click="changeSorting('processor_name')"><x-icon.sort :direction="$approved_by_sort_direction" height="26"
                        width="18" /></button>
            </div>
        </x-table.standard.th>
    ...
    <x-slot:table_data>
        @foreach ($this->payrolls as $payroll)
            <x-table.standard.row :class="in_array($payroll->id, $new_payroll_ids) ? '!bg-primary-100' : ''">
                {{-- Employee Name --}}
                ...
                @if (empty($view_details))
                    <x-table.standard.th class="max-w-40">
                        <button wire:click="showPayrollDetails('{{ $payroll->id }}')"
